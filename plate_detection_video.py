...
vidcap = cv2.VideoCapture('raw/r.mp4')
...
try:
			...
			while success:
				...
				for p in plates:
					...
					if len(plate_txt) > 0:
						...
						image_np = cv2.rectangle(image_np,(p[1], p[0]),(p[3], p[2]),(0,255,0),3)
						image_np = cv2.putText(image_np, plate_txt, (p[1], p[0]), font, 0.8 ,(0,0,255),2,cv2.LINE_AA)
				...
				for i in range(len(scores[0])):
					if scores[0][i] > 0.6:
						...
						image_np = cv2.rectangle(image_np,(y1,x1),(y2, x2),(255,0,0),3)
	...
	out = cv2.VideoWriter('dtcted/r.mp4', fourcc, 12, size)
	for i in range(len(image_array)):
		out.write(image_array[i])
	out.release()

print(len(image_array), " frames")
